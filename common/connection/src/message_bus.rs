...
pub struct ConnectionLane {
    ...
    input_channel: Receiver<Command>,
    output_channel: Sender<Command>,
    port: u16,
}
...
pub struct LaneCommunicator {
    write_to_lane: Sender<Command>,
    read_from_lane: Receiver<Command>,
    port: u16,
    lane_health: LaneHealth,
}

pub enum Command {
    SEND { key: Uuid, value: String },
}
...
/**
   A messaging channel for inter-node communication. Makes use of multiple (three) TCP Streams to avoid Head-of-line blocking
**/
impl MessageBus {
    ...
    pub fn new(
        connection_lanes: Vec<ConnectionLane>,
        ...
        for laneCom in connection_lane_comms.into_iter() {
            connection_lanes.insert(laneCom.port, laneCom);
        }
